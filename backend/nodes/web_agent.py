"""WebResearchAgent — sub-agent with independent tool set.
...
Independent from ArXivResearchAgent: disjoint tool set
...
LLM). The budget allows a possible tool-picking LLM call per
...
web_chunks list rather than crashing the whole job.
"""
import logging
...
# Independent tool set — disjoint from
...
async def web_agent_node(state: AcademicResearchState) -> dict:
    ...
    try:
        ...
        # Tavily failure -> warn + continue with arxiv-only. An empty
